...
- Callback entry points must be `noexcept` and carry `[[clang::nonblocking]]` in supporting Clang builds through a feature-tested project macro. That macro is `DUET_NONBLOCKING` in `duet/realtime/Callback.h`; it goes after the parameter list and its `noexcept`, and the target holding the callback links `duet::realtime`, which is what puts it under the sanitizer in the `linux-rtsan` configuration. The RealtimeSanitizer test must enter through that annotation and exercise the callback by offline rendering.
