...
- Spec boundary: the spec owns the AI seam's contracts and behavior; it binds but does not design the shared-undo mechanism (skb4tp), the technical audition mechanism (86t5lu), and conversation/Suggestion persistence (rquzdc). Constraint notes were left on skb4tp and 86t5lu.
- Suggestion emission: a single 'suggest' write-tool whose input schema IS the Suggestion; commentary is plain assistant text; at most one suggest per Task Run.
...
- Conversation and Suggestions are in-memory per app session in milestone one; persistence arrives with rquzdc.
...
- Cherry-pick granularity is the Suggestion ELEMENT (one human-meaningful change bundling several operations, independently applicable). Revision replaces a pending Suggestion (superseded); replying to a rejected one yields a new pending Suggestion.
- Test seams (agreed): the socket protocol (primary, no-LLM harness), the analysis routines as pure functions (worked examples incl. EBU Tech 3341), and the Suggestion manager's state machine.
