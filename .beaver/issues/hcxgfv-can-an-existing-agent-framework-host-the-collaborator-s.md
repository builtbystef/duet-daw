...
**The session tree maps onto Proposals.** `fork()` plus branching is structurally what node hll1mo's Proposal + A/B toggle needs — a branch that exists without mutating the trunk until accepted. Non-obvious fit, and the strongest single argument for pi. [docs/sdk.md; node hll1mo]
...
- Whether pi's session `fork()` semantics genuinely fit Proposals, or only appear to. Needs a prototype, not more reading.
