...
Grill session settled (2026-08-10). Mid-session the user supplied a Next.js UI mockup (~/Code/temp/duet-daw-ui-mockup, its own git repo) which I ran and explored in the browser; it becomes the visual reference. Colors/theme follow the mockup as closely as practical; layout may change; the Collaborator/Suggestion UX keeps the earlier prototype's mechanics (u64tso), not the mockup's simplified card.
...
AI ACCENT — TEAL, reserved exclusively for the Collaborator (✦ badges, suggestion glow/ghosts, ghost fader handles, commentary accents): approx #3fd0be on dark / #0e7c70 on light, tuned at the prototype. Deliberately brighter than the muted track-cyan/mint so it stands apart; avoids all four semantic hues (stale keeps amber). Violet/purple was explicitly rejected as AI-cliche.

COLLABORATOR/SUGGESTION UX — u64tso mechanics in full (per-element cherry-pick, stale-amber + redo-against-current, rejection-with-reason revision, ghost+glow clips, ghost fader + A/B mix toggle) wearing the mockup's panel/card styling; the audition button is labeled "Audition" per the glossary (mockup said "Preview"); History section (applied suggestions) kept, in-memory per session per js437t.
