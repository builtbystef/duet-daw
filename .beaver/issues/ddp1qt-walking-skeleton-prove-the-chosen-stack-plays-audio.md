...
3. Wave tracks silent, no error anywhere: AudioTrack::insertWaveClip stored the source path relative to the edit's TEMP subdirectory ('../bass.wav'), which resolves to a nonexistent file at playback. Diagnosed by reading the saved edit XML. Fix: clip->getSourceFileReference().setToFile(file, SourceFileReference::PathStyle::alwaysAbsolute, false) after every programmatic insert. TRAP FOR DUET: every programmatic clip insertion (including every Collaborator Suggestion that adds a clip) must pin the source reference deliberately.
