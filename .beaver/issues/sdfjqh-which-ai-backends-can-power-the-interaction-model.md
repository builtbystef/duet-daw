...
Which AI backends can power the interaction model settled in hll1mo — a Collaborator that sees the entire project state, consumes rendered audio on demand, and returns commentary and/or a Proposal (a bundle of structured project edits) from a non-blocking, cancelable, producer-initiated run? Evaluated against the roadmap's licensing posture (open source now, commercial edition possible later).
